...
app.post("/buildAndSponsorTx", async (req, res, next) => {
  try {
    ...
    const simpleTx: SimpleTransaction = await buildSimpleMoveCallTransaction(
      AccountAddress.from(req.body.sender),
      req.body.message
    );
...
async function buildSimpleMoveCallTransaction(
  ...
  return await aptosClient.transaction.build.simple({
    ...
    data: {
      function:
        "0xc13c3641ba3fc36e6a62f56e5a4b8a1f651dc5d9dc280bd349d5e4d0266d0817::message::set_message",
